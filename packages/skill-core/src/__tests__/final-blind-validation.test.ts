...
import { FINAL_BLIND_CORPUS, type FinalBlindScenario } from './final-blind-test-corpus'

describe('Final Untouched Blind Test Suite (100 Scenarios)', () => {
  ...
  it('evaluates frozen implementation in a single pass on pristine blind scenarios', () => {
    ...
    for (const scenario of FINAL_BLIND_CORPUS) {
      ...
      // 1. Check False Positives / Unsupported Out-of-Domain Requests
      if (scenario.expectedSkillId === null) {
        if (actualSkillId === null) {
          ...
          failures.push({
            ...
            reason: `DISCOVERY_FALSE_POSITIVE: Out-of-domain prompt falsely matched "${actualSkillId}" with score ${topCandidate?.score}`,
          })
        }
        continue
      }

      // 2. Check Skill Discovery Accuracy
      ...
      // 3. Check Parameter Resolution & Clarification Behavior
    ...
    const totalSupported = FINAL_BLIND_CORPUS.length - 20 // 80 supported
    const totalUnsupported = 20

    console.log('=================================================================')
    console.log('FINAL UNTOUCHED BLIND TEST RESULTS (100 SCENARIOS)')
    console.log('=================================================================')
    console.log(`Total Pristine Scenarios: ${FINAL_BLIND_CORPUS.length}`)
    console.log(`Skill Selection Accuracy: ${discoveryCorrect} / ${totalSupported} (${((discoveryCorrect / totalSupported) * 100).toFixed(1)}%)`)
    ...
    console.log(`Parameter Extraction Accuracy: ${parametersCorrect} / ${parametersCorrect + parametersIncorrect} (${(((parametersCorrect) / (parametersCorrect + parametersIncorrect)) * 100).toFixed(1)}%)`)
    console.log(`Clarification Rate: ${clarificationsCorrect} / ${clarificationsCorrect + clarificationsIncorrect} (${(((clarificationsCorrect) / (clarificationsCorrect + clarificationsIncorrect)) * 100).toFixed(1)}%)`)
